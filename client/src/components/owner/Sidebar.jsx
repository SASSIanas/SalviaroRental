...
const Sidebar = () => {
  ...
  <label htmlFor="image">
    <img 
      ...
      alt="" 
      className='md:h-24 md:w-24 h-10 w-10 rounded-full object-cover mx-auto border-2 border-gray-200'
    />
    <input 
      type="file" 
      id='image' 
    ...
    />
    <div className='absolute top-0 left-0 right-0 bottom-0 mx-auto bg-black/10 rounded-full hidden group-hover:flex items-center justify-center cursor-pointer'>
      <img src={assets.edit_icon} alt="" className='h-5 w-5' />
            ...
            <div className='w-full'>
                {ownerMenuLinks.map((link,index)=>(
                    ...
                    items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path === location.pathname ? 'bg-primary/10 text-primary' :
                         'text-gray-600'
                    }`}>
                        <img className='w-6 h-6' src={link.path === location.pathname ? link.coloredIcon : link.icon } alt="car icon" />
